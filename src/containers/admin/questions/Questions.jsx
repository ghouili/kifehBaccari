...
const TABLE_HEAD = ["Question", "Conclusion", "Recommendation", "Pratique", ""];
...
const Questions = () => {
  ...
  const fetchPratiques = async () => {
    const result = await axios.get(`http://localhost:5000/pratique`);

    setPratiques(result.data.data);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let url, result;
      if (formValues._id) {
        url = `${path}question/${formValues._id}`;
  ...
  const deleteData = async (id) => {
    ...
    if (willDelete) {
      const result = await axios.delete(`http://localhost:5000/question/${id}`);
